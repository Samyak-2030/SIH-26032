...
function renderManagedCentres() {
    centreTableBody.innerHTML = window.KisanSetuDemo.getCentres().map((centre) => `<tr><td><strong>${centre.name} - ${centre.location}</strong><small>Location: ${centre.location}</small></td><td>${centre.district}</td><td>${centre.capacity}</td><td>0</td><td class="admin-status active">${centre.status}</td><td>Static demo</td></tr>`).join('');
}
